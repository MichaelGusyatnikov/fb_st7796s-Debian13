...
Кроме того, полностью автоматизирован процесс установки - добавлен скрипт ```install.sh```, а также ```switch_to_landscape.sh``` и ```switch_to_upsidedown.sh``` для настройки экрана в альбомной и перевернутой ориентациях соответственно.
